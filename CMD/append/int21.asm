...
FCB_off equ filename_off
filename_off	dw	0
FCB_seg equ filename_seg
filename_seg	dw	0
basename_off	dw	0
new_filepath equ cmdline
...
;
int21:		pushf
		test	byte ptr cs:[handler], USERHNDL
		jz	int21cnt
		jmp	i21_jumporig
...
;
int21usr:	pushf
int21cnt:	test	word ptr cs:[append_state], APPEND_ENABLED
		jz	i21_jump

		cmp	ah, 0Fh	; Open file using FCB
		je	i2123
		cmp	ah, 23h	; Get file size for FCB
		je	i2123
		cmp	ah, 3Dh	; Open existing file
		jne	i21_ck6c
		jmp	i213d
i21_ck6c:	cmp	ax, 6C00h	; Extended open/create
		jne	i21_ck_x
		jmp	i216c

i21_ck_x:	test	word ptr cs:[append_state], APPEND_EXTENDD
		jz	i21_jump

		cmp	ah, 11h	; Find first matching file using FCB
		je	i2111
		cmp	ax, 4B00h	; Exec - Load and execute program
		jne	i21_ck_4b03
		jmp	i214b00
i21_ck_4b03:	cmp	ax, 4B03h	; Exec - Load overlay
		jne	i21_ck_4e
		jmp	i214b03
i21_ck_4e:	cmp	ah, 4Eh	; FindFirst - Find first matching file
		jne	i21_jump
		jmp	i214e

i21_jump:	test	byte ptr cs:[handler], USERHNDL
		jz	i21_jumporig
		popf
		;jmp	far [cs:usr_int21]
		jmp	dword ptr cs:[usr_int21]
i21_jumporig:	popf
...
i2111:		push	bx
		...
		test	byte ptr ds:[bx+6], 08h	; Volume label?
		pop	bx
...
; ---------------------------------------------------------------------------
;
i216c:		test	dl, 10h		; Create if file doesn't exist?
...
; ---------------------------------------------------------------------------
;
i214e:		test	cl, 08h		; Is it a volume label?
		jz	i214b03			; No, go on
...
i213d_srch:	test	word ptr cs:[append_state], APPEND_ENVIRON	; /E?
		jz	i213d_noenv
...
i213d_noenv:	mov	ds, cs:[filename_seg]
		...
		jne	i213d_basename
		test	word ptr cs:[append_state], APPEND_SRCHDRV
		jnz	i213d_basename
		jmp	i213d_done
...
i213d_basename: call	basename
		...
		test	word ptr cs:[append_state], APPEND_SRCHPTH ;  /PATH:ON
		jnz	i213d_srch2
		jmp	i213d_done
...
i214b00_srch:	test	word ptr cs:[append_state], APPEND_ENVIRON	; /E?
		jz	i214b00_noenv
		call	copy_environ	; Copy env APPEND to append_path
...
i214b00_exec:	push	dx			; Restores caller's DTA
		push	ds
		mov	ah, 1Ah
		mov	dx, word ptr cs:[DTA]
		mov	ds, word ptr cs:[DTA+2]
		call	call_int21
		pop	ds
		pop	dx
...
cint21_usr:	call	dword ptr cs:[usr_int21]
		ret
...
set_rfn:	push	ax
		...
		cmp	ax, 4E00h	; AL nonzero has special meaning for
		jne	sr_copy		; setrfn in function FindFirst.

		mov	ah, 2Fh		; Get current Disk Transfer Area
...
clear_rfnstate: push	bx
		push	es
		les	bx, cs:[rfnstate]
		and	byte ptr es:[bx], 0 ; Clear state in caller's PSP
		pop	es
		pop	bx
		ret
...
get_rfnstate:	push	ax
		...
		call	call_int21
		mov	word ptr cs:[rfnstate], 003Dh
		mov	word ptr cs:[rfnstate+2], bx
		pop	bx
		pop	ax
		ret
...
;	    files
;
check_error:	cmp	al, 02h		; File not found
		jnc	ce_ret
		cmp	al, 03h		; Path not found
		jnc	ce_ret
...
sn_getdrive:	mov	al, es:[di]			; Stores drive letter
		mov	byte ptr ds:[si], al		; in buffer
		inc	si
		mov	byte ptr ds:[si], ':'
		inc	si
		toupper al,4
		sub	al, 40h		; 'A' == 1, ... ('A' == 0x41 ASCII)

sn_setdrive:	call	set_drive	; Set new drive in FCB

		mov	byte ptr ds:[si], '\'	; Add leading backslash
		inc	si
...
ff_eostr:	or	cx, cx
		jz	ff_empty
		mov	byte ptr ds:[di], 5Ch	; Add '\' to the end
		inc	di
